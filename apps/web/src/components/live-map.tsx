...
function clampToMapX(value: number, mapWidth: number) {
  return clamp(value, BLIP_RADIUS, mapWidth - BLIP_RADIUS);
}

function clampToMapY(value: number, mapHeight: number) {
  return clamp(value, BLIP_RADIUS, mapHeight - BLIP_RADIUS);
}

function resolveMapBounds(
  canvasSize: { width: number; height: number } | null | undefined,
  background: MapBackgroundConfig | null | undefined,
  rooms: RoomZone[],
  mapProps: MapPropElement[],
): ResolvedMapBounds {
  let width = canvasSize?.width && canvasSize.width > 0 ? canvasSize.width : DEFAULT_MAP_WIDTH;
  let height = canvasSize?.height && canvasSize.height > 0 ? canvasSize.height : DEFAULT_MAP_HEIGHT;

  if (background) {
    width = Math.max(width, background.x + background.w);
    height = Math.max(height, background.y + background.h);
  }

  for (const room of rooms) {
    width = Math.max(width, room.x + room.w);
    height = Math.max(height, room.y + room.h);
  }

  for (const prop of mapProps) {
    width = Math.max(width, prop.x + prop.w);
    height = Math.max(height, prop.y + prop.h);
  }
...
export function LiveMap({
  ...
  const [stageWidth, setStageWidth] = useState(DEFAULT_MAP_WIDTH);
  ...
  const mapBounds = useMemo(
    () => resolveMapBounds(canvasSize, background, rooms, mapProps),
    [background, canvasSize, mapProps, rooms],
  );
  const stageHeight = Math.max(280, Math.floor(stageWidth * (mapBounds.height / mapBounds.width)));
  const fitScale = Math.min(stageWidth / mapBounds.width, stageHeight / mapBounds.height);
  ...
  const getFittedViewport = (scale = 1): LiveMapViewport => {
    const scaledWidth = mapBounds.width * fitScale * scale;
    const scaledHeight = mapBounds.height * fitScale * scale;
    return {
      x: Math.round((stageWidth - scaledWidth) / 2),
      y: Math.round((stageHeight - scaledHeight) / 2),
      scale,
    };
  };
  ...
  function clampViewport(next: LiveMapViewport) {
    ...
    const scaledWidth = mapBounds.width * fitScale * zoom;
    const scaledHeight = mapBounds.height * fitScale * zoom;
    const minX = Math.min(0, stageWidth - scaledWidth);
    const minY = Math.min(0, stageHeight - scaledHeight);
    const maxX = scaledWidth > stageWidth ? 0 : Math.round((stageWidth - scaledWidth) / 2);
    const maxY = scaledHeight > stageHeight ? 0 : Math.round((stageHeight - scaledHeight) / 2);
  ...
  }, [fitScale, mapBounds.height, mapBounds.width, stageHeight, stageWidth]);

  useEffect(() => {
    if (!storageKey) {
      setViewport(clampViewport(getFittedViewport(1)));
      setBlipOverrides({});
      return;
    }
    ...
    const nextViewport = persisted.viewport
      ...
      : clampViewport(getFittedViewport(1));
    ...
    setBlipOverrides(interactive ? nextBlips : {});
  }, [interactive, storageKey, fitScale, mapBounds.height, mapBounds.width, stageHeight, stageWidth]);
  ...
  useEffect(() => {
    ...
    const payload: PersistedLiveMapState = {
      viewport,
      blips: interactive ? blipOverrides : undefined,
    };
  ...
  const activeDisconnectPings = useMemo(() => {
    ...
    disconnectPings.forEach((ping) => {
      ...
      resolved.push({
        ...
        x:
          typeof ping.x === "number" && Number.isFinite(ping.x)
            ? clampToMapX(ping.x, mapBounds.width)
            : fallback.x,
        y:
          typeof ping.y === "number" && Number.isFinite(ping.y)
            ? clampToMapY(ping.y, mapBounds.height)
            : fallback.y,
        progress,
      });
    });
  ...
  function adjustZoom(multiplier: number) {
    const nextZoom = clamp(viewport.scale * multiplier, MIN_ZOOM, MAX_ZOOM);
    const centerX = stageWidth / 2;
    const centerY = stageHeight / 2;
    const oldScale = fitScale * viewport.scale;
    ...
    setClampedViewport({
      scale: nextZoom,
      x: Math.round(centerX - worldPoint.x * nextScale),
      y: Math.round(centerY - worldPoint.y * nextScale),
    });
  }
    ...
    const tick = (frameAt: number) => {
      ...
      if (horizontal !== 0 || vertical !== 0) {
        ...
        if (activeRoom) {
          ...
          const activePoint =
            override
              ? {
                  ...
                  x: clampToMapX(override.x, mapBounds.width),
                  y: clampToMapY(override.y, mapBounds.height),
                }
              : defaultPoint;
          ...
          const nextX = clampToMapX(moved.x, mapBounds.width);
          const nextY = clampToMapY(moved.y, mapBounds.height);
    ...
    <Paper ref={wrapperRef} elevation={0} sx={{ p: 2, borderRadius: 4, border: 1, borderColor: 'divider', overflow: 'hidden' }}>
      ...
      >
        <Layer>
          ...
          <Rect
            x={16}
            y={16}
            width={mapBounds.width - 32}
            height={mapBounds.height - 32}
            cornerRadius={20}
          ...
          {mappedLocations.map((location) => {
            ...
            const basePoint = overridden
              ? {
                  ...
                  x: clampToMapX(overridden.x, mapBounds.width),
                  y: clampToMapY(overridden.y, mapBounds.height),
                }
              : defaultPoint;
            ...
            const isCurrentPlayer = currentPlayerId === location.employeeId;
            const labelText = location.employeeId;
            const labelFontSize = isCurrentPlayer ? 16 : 11;
            const estimatedLabelWidth = Math.max(42, Math.round(labelText.length * (isCurrentPlayer ? 9 : 6.5)));
            const labelX = clamp(activePoint.x + 8, 6, mapBounds.width - estimatedLabelWidth - 12);
            const preferredLabelY = activePoint.y - (isCurrentPlayer ? 28 : 16);
            const labelY = preferredLabelY < 8 ? activePoint.y + 10 : preferredLabelY;
            const labelHeight = isCurrentPlayer ? 20 : 14;
              ...
              <Fragment key={`${location.employeeId}-marker`}>
                <Circle
                  ...
                  dragBoundFunc={(position) => ({
                    x: clampToMapX(position.x, mapBounds.width),
                    y: clampToMapY(position.y, mapBounds.height),
                  })}
                  onDragEnd={(event) => {
                    if (!canDragMarker) {
                      return;
                    ...
                    const rawX = clampToMapX(event.target.x(), mapBounds.width);
                    const rawY = clampToMapY(event.target.y(), mapBounds.height);
                ...
                <Rect
                  ...
                  height={labelHeight}
                  cornerRadius={isCurrentPlayer ? 7 : 5}
                  fill={isCurrentPlayer ? "rgba(15,23,42,0.96)" : "rgba(15,23,42,0.78)"}
                  stroke={isCurrentPlayer ? "rgba(250,204,21,0.96)" : "rgba(148,163,184,0.62)"}
                  strokeWidth={isCurrentPlayer ? 1.8 : 1}
                  listening={false}
                />
                <Text
                  x={labelX}
                  ...
                  fill={isCurrentPlayer ? "rgba(255,255,255,0.99)" : style.text}
                  shadowColor="rgba(2,6,23,0.92)"
                  shadowBlur={isCurrentPlayer ? 3 : 1}
                  shadowOffsetX={0}
                  shadowOffsetY={1}
                  listening={false}
                />
      ...
      {interactive ? (
        <Stack direction="row" spacing={1} sx={{ mt: 2, flexWrap: 'wrap', alignItems: 'center' }}>
          <Button size="small" variant="outlined" onClick={() => adjustZoom(1 / ZOOM_STEP)}>
            Zoom Out
          </Button>
          <Button size="small" variant="outlined" onClick={() => adjustZoom(ZOOM_STEP)}>
            Zoom In
          </Button>
          <Button size="small" variant="outlined" onClick={() => setClampedViewport(getFittedViewport(1))}>
            Reset View
          </Button>
        ...
        <Stack direction="row" spacing={1} sx={{ mt: 2, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <Typography variant="caption" color="text.secondary">
            Read-only markers. Drag map to pan and use zoom controls or wheel to inspect the full floor plan.
          </Typography>
        </Stack>
      )}
