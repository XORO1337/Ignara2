...
export class VoiceGateway implements OnModuleDestroy {
  ...
  private globalKey(orgId: string) {
    return `org:${orgId}:voice:global`;
  }
  ...
  private async handleJoin(socket: Socket, payload: VoiceJoinPayload) {
    ...
    if (mode === "room") {
      if (!roomId) {
        this.emitVoiceError(socket, {
          reason: "invalid-payload",
          message: "Voice room join denied: roomId is required in ROOM mode.",
        });
        return;
      }
    ...
    const channelId = mode === "global" ? this.globalKey(orgId) : this.roomKey(orgId, roomId!);
    const members = this.roomMembers.get(channelId) ?? new Map<string, string>();
    const peers = [...members.keys()].filter((peerId) => peerId !== employeeId);

    members.set(employeeId, socket.id);
    this.roomMembers.set(channelId, members);
    this.participantBySocketId.set(socket.id, { orgId, employeeId, mode, roomId: mode === "room" ? roomId : undefined, channelId });

    socket.join(channelId);
    socket.emit("voice:peers", { roomId: mode === "room" ? roomId! : "global", peers });
    socket.to(channelId).emit("voice:peer-joined", { employeeId });
  }
  ...
  private leaveRoom(socket: Socket) {
    ...
    const members = this.roomMembers.get(participant.channelId);
    if (members) {
      members.delete(participant.employeeId);
      if (members.size === 0) {
        this.roomMembers.delete(participant.channelId);
      }
    }

    socket.leave(participant.channelId);
    this.participantBySocketId.delete(socket.id);
    socket.to(participant.channelId).emit("voice:peer-left", { employeeId: participant.employeeId });
  }
