...
export function MapEditorCanvas({ locations }: MapEditorCanvasProps) {
  ...
  const canvasWidth = Number.isFinite(canvasSize.width) && canvasSize.width > 0 ? canvasSize.width : DEFAULT_WIDTH;
  const canvasHeight = Number.isFinite(canvasSize.height) && canvasSize.height > 0 ? canvasSize.height : DEFAULT_HEIGHT;

  const canvasAspectRatio = useMemo(() => {
    return canvasHeight / canvasWidth;
  }, [canvasHeight, canvasWidth]);
  const fitScale = useMemo(
    () => Math.min(stageSize.width / canvasWidth, stageSize.height / canvasHeight),
    [canvasHeight, canvasWidth, stageSize.height, stageSize.width],
  );
  const minScale = Math.max(MIN_CANVAS_SCALE, fitScale * 0.35);
  const maxScale = Math.max(MAX_CANVAS_SCALE, fitScale * 8);
  ...
  useEffect(() => {
    ...
    if (clamped.x !== viewport.x || clamped.y !== viewport.y || clamped.scale !== viewport.scale) {
      setViewport(clamped);
    }
  }, [canvasHeight, canvasWidth, stageSize, viewport]);

  useEffect(() => {
    const fitKey = `${canvasWidth}x${canvasHeight}`;
    if (autoFitKeyRef.current === fitKey) {
      return;
    }

    autoFitKeyRef.current = fitKey;
    setViewport(clampViewport(Math.round((stageSize.width - canvasWidth * fitScale) / 2), Math.round((stageSize.height - canvasHeight * fitScale) / 2), fitScale));
  }, [canvasHeight, canvasWidth, fitScale, stageSize.height, stageSize.width]);
  ...
  function clampViewport(x: number, y: number, scale: number) {
    ...
    const maxX = scaledWidth > stageSize.width ? 0 : Math.round((stageSize.width - scaledWidth) / 2);
    const maxY = scaledHeight > stageSize.height ? 0 : Math.round((stageSize.height - scaledHeight) / 2);
  ...
  function handleWheel(event: KonvaEventObject<WheelEvent>) {
    ...
    const nextScale = event.evt.deltaY > 0 ? oldScale / ZOOM_STEP : oldScale * ZOOM_STEP;
  ...
  function adjustZoom(multiplier: number) {
    const nextScale = Math.max(minScale, Math.min(maxScale, viewport.scale * multiplier));
    ...
    setViewport(
      clampViewport(
        Math.round(centerX - worldX * nextScale),
        Math.round(centerY - worldY * nextScale),
        nextScale,
      ),
    );
  }
      ...
      <Stack direction="row" spacing={1} sx={{ mt: 2, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <Button
          ...
          onClick={() =>
            setViewport(
              clampViewport(
                Math.round((stageSize.width - canvasWidth * fitScale) / 2),
                Math.round((stageSize.height - canvasHeight * fitScale) / 2),
                fitScale,
              ),
            )
          }
        ...
        <Typography variant="caption" color="text.secondary">
          Drag empty map to pan. Default view fits the full floor plan; use wheel or controls to zoom.
        </Typography>
      </Stack>
    </Paper>
  );
